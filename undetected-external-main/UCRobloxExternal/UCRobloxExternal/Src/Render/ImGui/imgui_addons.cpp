...
bool Menu::BeginChild(const char* Label, const ImVec2& SizeArg)
{
    ...
    ImGui::BeginChild(StrLabel.c_str(), SizeArg, ImGuiChildFlags_None, ImGuiWindowFlags_NoBackground);
    ...
    ImGui::BeginChild((StrLabel + "##Body").c_str(), ImVec2(Size.x - Border * 2.0f, Size.y - (HasHeader ? HeaderHeight : 0.0f) - Border), ImGuiChildFlags_AlwaysUseWindowPadding, ImGuiWindowFlags_NoBackground);
...
bool Menu::CheckBox(const char* label, bool* checked)
{
    ...
    if (*checked)
    {
        ...
        window->DrawList->AddRectFilled(checkBB.Min, checkBB.Max, ImColor(0, 0, 0, 70), style.FrameRounding);
    }
    else
    {
        window->DrawList->AddRectFilled(checkBB.Min, checkBB.Max, boxCol, style.FrameRounding);
        window->DrawList->AddRectFilled(checkBB.Min, checkBB.Max, IM_COL32(28, 28, 28, 255), style.FrameRounding);
    }
...
bool Menu::ColorEdit4(const char* name, float col[4])
{
    ...
    window->DrawList->AddRectFilled(bb_min, bb_max, ImColor(0, 0, 0, 70), style.FrameRounding);
...
bool Menu::SliderScalar(const char* Label, ImGuiDataType DataType, void* PData, const void* PMin, const void* PMax, const char* Format)
{
    ...
    Window->DrawList->AddRectFilled(SliderBB.Min, SliderBB.Max, ImColor(0, 0, 0, 70), Style.FrameRounding);
...
bool Menu::KeyBind(const char* StrId, ImGuiKey* Key)
{
    ...
    auto GetShortKeyName = [](ImGuiKey k) -> std::string {
        if (k == ImGuiKey_None)        return "Unbound";
        if (k == ImGuiKey_MouseLeft)   return "LMB";
        if (k == ImGuiKey_MouseRight)  return "RMB";
        if (k == ImGuiKey_MouseMiddle) return "MMB";
        if (k == ImGuiKey_MouseX1)     return "MB4";
        if (k == ImGuiKey_MouseX2)     return "MB5";
        if (k == ImGuiKey_LeftShift)   return "LShift";
        if (k == ImGuiKey_RightShift)  return "RShift";
        if (k == ImGuiKey_LeftCtrl)    return "LCtrl";
        if (k == ImGuiKey_RightCtrl)   return "RCtrl";
        if (k == ImGuiKey_LeftAlt)     return "LAlt";
        if (k == ImGuiKey_RightAlt)    return "RAlt";
        if (k == ImGuiKey_Space)       return "Space";
        if (k == ImGuiKey_Enter)       return "Enter";
        if (k == ImGuiKey_Backspace)   return "Bkspc";
        if (k == ImGuiKey_Tab)         return "Tab";
        if (k == ImGuiKey_Escape)      return "Esc";
        if (k == ImGuiKey_Delete)      return "Del";
        if (k == ImGuiKey_Insert)      return "Ins";
        if (k == ImGuiKey_Home)        return "Home";
        if (k == ImGuiKey_End)         return "End";
        if (k == ImGuiKey_PageUp)      return "PgUp";
        if (k == ImGuiKey_PageDown)    return "PgDn";
        if (k == ImGuiKey_CapsLock)    return "Caps";
        std::string n = GetKeyName(k);
        if (n.size() > 6) n = n.substr(0, 6);
        return n;
    };

    std::string keyStr = GetShortKeyName(*Key);
    const char* KeyName = keyStr.c_str();

    ImVec2 TextSize = CalcTextSize(KeyName);
    float BoxW = ImMax(40.0f, TextSize.x + 10.0f);
    ImVec2 BoxSize(BoxW, 15.0f);
    ImVec2 Pos = Window->DC.CursorPos - ImVec2(37.0f + (BoxW - 40.0f), 0.0f);
    ...
    const char* DisplayName = IsSelecting ? "..." : KeyName;
    ImVec2 DisplaySize = CalcTextSize(DisplayName);
    ImVec2 TextPos = FrameBb.Min + ImVec2((BoxSize.x - DisplaySize.x) / 2.0f, (BoxSize.y - DisplaySize.y) / 2.0f - 1.0f);

    DrawList->PushClipRect(FrameBb.Min + ImVec2(2, 1), FrameBb.Max - ImVec2(2, 1), true);
    Menu::DrawLabelShadow(DrawList, TextPos, ItAnim->second.Label, DisplayName);
...
bool Menu::KeyBindEx(const char* StrId, ImGuiKey* Key, ImKeyBindMode* Mode, const ImVec2& SizeArg)
{
    ...
    {
        ImGui::BeginChild("popup_rect", ImVec2(CalcTextSize("ToggleHold").x + Style.ItemSpacing.x * 2.0f - 8.0f, GetFontSize()), ImGuiChildFlags_None, ImGuiWindowFlags_NoBackground);
...
// Fallback logic for int* bindings
bool Menu::KeyBind(const char* StrId, int* K) {
    ImGuiKey key = (ImGuiKey)*K; // Not perfectly accurate mapping, but ImGui works for basics.
    // For proper support we should manually map it back and forth, but let's just use ImGuiKey directly for now
    // Actually, their original system uses GetAsyncKeyState virtual keys. We might need a small mapping.
    // To make things simple and since their user code sets an `int`, we'll pass an intermediate ImGuiKey
    // and store the mapped VK back to *K.
    
    // We need to map *K (VK code) to ImGuiKey for display if it's already set.
    ImGuiKey displayKey = ImGuiKey_None;
    if (*K != 0) {
        // Find matching key
        for (int i = ImGuiKey_NamedKey_BEGIN; i < ImGuiKey_NamedKey_END; i++) {
            if (ImGuiKeyToVK((ImGuiKey)i) == *K) {
                displayKey = (ImGuiKey)i;
                break;
            }
        }
    }
    
    bool changed = Menu::KeyBind(StrId, &displayKey);
    if (changed) {
        *K = ImGuiKeyToVK(displayKey);
    }
    return changed;
}

bool Menu::KeyBindEx(const char* StrId, int* K, ImKeyBindMode* Mode, const ImVec2& SizeArg) {
    ImGuiWindow* Window = GetCurrentWindow();
    if (Window->SkipItems) return false;
    ...
    bool Result = Menu::KeyBind(StrId, K);
