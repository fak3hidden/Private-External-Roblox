...
namespace Menu
{
    ...
    // Fallback for int* K since the user code uses raw int instead of ImGuiKey
    bool KeyBind(const char* StrId, int* K);
    bool KeyBindEx(const char* StrId, int* K, ImKeyBindMode* Mode, const ImVec2& SizeArg = ImVec2(0, 0));
